...
set :trans,0.0 #set initial slider recorded position
...
define :rescale do |n|
  ...
  return ((n+1)/2)*12 #scale to +- octave
end
...
live_loop :slider do
  ...
  scaled = rescale(hry)
  osc_send "127.0.0.1",9000,"/test/fader2",hry
  osc_send "127.0.0.1",9000,"/test/scale1",scaled
  puts "slider is ",scaled
...
live_loop :toggle do
  ...
  if b[0]>0 #if it is active (pressed and on)
    use_synth :tri
    x=play note(:c3)+get(:trans),sustain: 1000 #start a very long note and record referece in x
    ...
    in_thread do #a thread containing a continuously running loop
      loop do
        ...
        control get(:x),note: note(:c3)+get(:trans),note_slide: 0.1
