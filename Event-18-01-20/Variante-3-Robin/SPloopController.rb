...
live_loop :pon do
  ...
  if b[0]==1
    ...
    ns= r[2]
    puts "on channel "+ns, get[("c"+ns).to_sym]
    set ("c"+ns).to_sym,1
...
live_loop :poff do
  ...
  if b[0]==0
    r=parse_sync_address "/osc*/kinect/*"
    ns= r[2]
    puts "off channel "+ns, get[("c"+ns).to_sym]
    set ("c"+ns).to_sym,0
  end
end
...
with_fx :gverb,room: 15,mix: 0.8,amp: 0.5 do
  live_loop :alwaysplaying,sync: :metro do #runs continuously playing
    use_synth :fm
    play :c3,release: 1,amp: 1,pan: -0.8 if get(:c99)==1
    play :c4,release: 2,amp: 0.2,pan: -0.8 if get(:c99)==1
    osc "/1/led1",1  if get(:c99)==1#switch on large LED on TouchOSC screen
    sleep 1
    osc "/1/led1",0  #switch on large LED on TouchOSC screen
    sleep 1
    play :c3,release: 2,amp: 1,pan: 0.8 if get(:c99)==1
    osc "/1/led1",1  if get(:c99)==1#switch on large LED on TouchOSC screen
    sleep 1
